...
app.get('/seller/manage_order',async function (req,res){
    await storage.init( /* options ... */ );
    var sellerid = await storage.getItem('sellerid')
    var query = `select product.* , buylist.* from buylist inner join product on buylist.product_id = product.id where product.seller_id= ${sellerid}`
    con.query(query, function (error,result,index){
        if (error)throw error;
        console.log(result);
...
app.get('/seller/manage_order/:status/:id', function (req,res){
    ...
    var query = `update buylist set status='${status}' where id=${id}`
    con.query(query,function (error,result,index){
        if (error)throw error;
...
app.get('/user_buylist',async function(req, res){
   ...
   // 
    con.query(query, function(error,result,index){
        ...
        var total = 0
        res.render('user/user_buylist',{result,total});
    })
})
